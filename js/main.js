...
// define a class to use for our game objects. 1:21:00
class Crawler {
...
function detectHit(objectOne, objectTwo) {
    ...
    const top = objectOne.y + objectOne.height >= objectTwo.y; /*if the bottom of the hero (y+height) is >= the top of the oger, there was a collision */
...
// create a gameloop -- run the business logic of the game and be called by a setInterval. 1:28:58
const gameInterval = setInterval(gameloop, 80);
function gameloop() {
    // update the inputs
    movementHandler(); // checking the input object
    // clear the canvas to rerender
    ctx.clearRect(0, 0, canvas.width, canvas.height); /*THis line is saying clear the canvas. starting at x=0 y=0 all the way thru to the x=max and y=max (so the entire canvas) */
